...
const RoleCard = ({ title, description, icon: Icon, type, onClick }) => {
    ...
    return (
        ...
        ${isDoctor ? 'from-primary/5 via-transparent to-transparent' : 'from-orange-100/50 via-transparent to-transparent'}`}
            />
        ...
        ${isDoctor ? 'bg-primary' : 'bg-orange-300'}`}
            />
          ...
          ${isDoctor ? 'bg-secondary-dark/20 group-hover:bg-primary group-hover:shadow-primary/30' : 'bg-orange-50 group-hover:bg-orange-500 group-hover:shadow-orange-500/30'}`}>
                    <Icon className={`w-7 h-7 transition-colors duration-300 ${isDoctor ? 'text-primary-dark group-hover:text-white' : 'text-orange-600 group-hover:text-white'}`} />
                </div>
